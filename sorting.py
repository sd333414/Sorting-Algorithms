    ...
    <input type="button" value="Jumble" onclick="document.getElementById('myContent').innerHTML='{1}'"/>
        ...
        if len(UnsortedList) > 1:
            ...
            for i in range(len(UnsortedList)):
                ...
                if i != pivotIndex:
                    if UnsortedList[i] < UnsortedList[pivotIndex]:
                        leftPart.append(UnsortedList[i])
                    else:
                        rightPart.append(UnsortedList[i])

            yield from quickSort(leftPart, 2)
            yield from quickSort(rightPart, 2)
            UnsortedList[:] = leftPart + [UnsortedList[pivotIndex]] + rightPart
